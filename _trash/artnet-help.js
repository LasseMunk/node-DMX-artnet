...
artnet.set(1, 255, function (err, res) {
  ...
  The set method can set multiple channels at once: Use an array to set subsequent channels...
...
Additionally, you can send trigger macros to devices.
 // set channel 100 to 10, channel 101 to 20 and channel 102 to
...
artnet.trigger(1, 3);
// Send key 2, subkey 71 (the letter 'G') to ArtNet Devices re
...
set( [ [ uint15 universe , ] uint9 channel , ] array[uint8] values [ , function(err, res) callback ] ) Every parameter except the value(s) is optional. If you supply a universe you need to
supply the channel also. Defaults: universe = 0, channel = 1
...
Sends an ArtNet ArtTrigger packet. ArtTriggers are typically device specific and perform
functions like starting and stopping shows.
Every parameter except the key is optional. If you supply an oem , you need to supply a
subkey also. Defaults:
oem = 0xFFFF subkey = null
Callback is called with (error, response) params.
trigger s are NEVER throttled, as they are time sensitive. They are always sent
immediately upon processing.
close( )
Closes the connection and stops the send interval.
setHost( string host )
Change the Art-Net hostname/address after initialization setPort( number port )
